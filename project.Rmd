...
wa <- read.csv('selected.csv', na.strings = "")
wa <- subset(wa, select = -X)
# http://simplemaps.com/data/us-cities
city = read.csv('uscitiesv1.3.csv', na.strings = "")
...
After using log to scale contb_receipt_amt, the contribution seems to become normal.
And then I will try to use shapiro test to test the log normality of contb_teceipt_amt.
As the chart shows, the distribution of contb_receipt_amt is not smooth and have several peak.
Accordingly, I decide to facet the chart to three peices.
...
Though the W is almost 1, but the p-value is too small.
That is to say the Shapiro-Wilk normality test shows that contb_receipt_amt is
...
The faceting charts shows that the contribution are focus on several point, such as 25 50, 100.
It make sense, people will seldom donate money like 7, 21 or 123.
The another thing we can notice it that the majority of contributions are small money.
Besides, there is a small peak at 2700 since this is the ceiling of contribution.
...
There is some NA, maybe value missing or transgender or whatever.
Anyway, I will just remove the NA when analyzing something relating to gender.
...
Here is the chart about candidate gender count.
We can see that the number of male candidates overwhelms female's.
...
It can be seen from the graph that the number of republican candidate overwhelm
democrat's.

But as we all konw now, the Democrat, Hillary win the Washington state.

```{r echo=FALSE}
plot_bar(wa, "election_tp")
```

The graph shows that the contribution of Primary is more than twice
as much as the double general.
...
According to the figure, the top2 occupation retired and notemployed are far
more than others. 

So, I decide to see what percentile are the top2 occupation.

```{r echo=FALSE}
wa.amt_by_occupation <- wa %>%
  group_by(contbr_occupation) %>%
  summarise(n = n()) %>%
  arrange(n)

subset(wa.amt_by_occupation, contbr_occupation == "RETIRED", "n") / 289340
subset(wa.amt_by_occupation, contbr_occupation == "NOT EMPLOYED", "n") / 289340
```

That is high.
It seems not good that 1/3 of the contribution are made by retired and not 
employed people since they exactly do not have income. 
That figure is obviously higher than their percent in total population.
In other word, the young employees are less active in election.
It is weird.
...
I just want to see the distribution of zipcode here. I assumed that it will just
look like the city freq chart, having a few max outliers.
But it is more smooth than city freq chart.
...
From the figures, we can see contributor's demographic distribution divided by
zipcode. 
...
Shapiro-Wilk normality test shows that none of them is normal distribution.
...
No suprise, most of the percent_white are 75 percent or more.
...
First, I just output the correlation matrix and then plot it, the higher the
correlation, the bigger the circle. 

With the help of the matrix plot, I can easily distinguish those high
correlations and confirm the real correlation value by the correlation matrix.

Here are some interesting correlation:
...
And then I use ggpair to make a matrix of plots to visualize the relation
between all pair of variables and the distribution of variables.

```{r echo=FALSE}
plot_point <- function(df, x, y, alpha) {
...
```{r echo=FALSE}
p1 <- plot_point(wa, "total_population", "median_age", 0.02)
p2 <- plot_point(wa, "percent_white", "percent_black", 0.02)
p3 <- plot_point(wa, "percent_asian", "per_capita_income", 0.02)
p4 <- plot_point(wa, "percent_hispanic", "median_rent", 0.02)
grid.arrange(p1, p2, p3, p4)
```

Some interesting demographic relation.
We can conclude from the first graph that the big city have more young people.
The others are aboout ethnic.
...
```{r echo=FALSE}
subset(wa, election_tp == "P2020")
```

We can see that the only one contribution to 2020 primary election is from a
retired to Graham, Lindsey O. with $2700.
...
From this chart, We can see that the Republican have higher average
contribution.
...
```{r echo=FALSE}
p1 <- ggplot(wa.amt_by_party, aes(party, n)) +
  geom_bar(stat = 'identity', aes(fill = party)) +
  scale_fill_manual("legend",
                     values = c("blue", "green", "red"))
p2 <- ggplot(wa.amt_by_party, aes(party, amt_sum)) +
  geom_bar(stat = 'identity', aes(fill = party)) +
  scale_fill_manual("legend",
                     values = c("blue", "green", "red"))
p3 <- ggplot(wa.amt_by_party, aes(party)) +
...
But from these charts we can illustrates that Democrat overwhlems Republican
about the number of contribution so that Democrat also overwhlems Republican about
the total contribution.

We can also cross-validate from the last bar chart that the average contribution
of Republican is highr than Democrat's. But the other parties have the highest
...
p1 <- ggplot(wa.amt_by_contbr_gender, aes(contbr_gender, amt_sum)) +
  ...
  scale_fill_manual("legend",
                     values = c("red", "blue"))
p2 <- ggplot(wa.amt_by_contbr_gender, aes(contbr_gender, amt_sum)) +
  geom_bar(stat = 'identity', aes(fill = contbr_gender)) +
  scale_fill_manual("legend",
                     values = c("red", "blue"))
p3 <- ggplot(wa.amt_by_contbr_gender, aes(contbr_gender)) +
...
The charts clearly show that male outperform female on all the number of
contribution, the tota contribution and the average contribution.
...
```{r echo=FALSE}
p1 <- plot_point(wa.amt_by_income, "per_capita_income", "n", 0.1)
p2 <- plot_point(wa.amt_by_income, "per_capita_income", "amt_sum", 0.1)
p3 <- plot_point(wa.amt_by_income, "per_capita_income", "amt_mean", 0.1)
grid.arrange(p1, p2, p3, ncol = 1)
```

It shows that people have around 50000 income make most contributions, but
people have about 70000 income make the most total contribution.

And the more per capita income, the more contribution, but the average
contribution almost the same.

The median contribution peak at low income seems very strange.

```{r echo=FALSE}
subset(wa, contb_receipt_amt > 2000 & per_capita_income < 20000,
       select = c(cand_first_nm, contbr_gender,
                  contb_receipt_amt, per_capita_income, contbr_occupation))
```

Acoordingly, I output these observation with some selected variables.

The figures shows that most of the contributions are donated to Hillary and 
Trump!
What is more, most of the Hillary contributor did not leave their
occupation information. In contrast, among 9 Trump contributors, 5 are farmer!
...
```{r echo=FALSE}
p1 <- ggplot(wa.amt_by_age, aes(median_age, n)) +
  scale_x_continuous(breaks = seq(20, 60, 10)) +
  geom_point()
p2 <- ggplot(wa.amt_by_age, aes(median_age, amt_sum)) +
  scale_x_continuous(breaks = seq(20, 60, 10)) +
  geom_point()
p3 <- ggplot(wa.amt_by_age, aes(median_age, amt_mean)) +
  scale_x_continuous(breaks = seq(20, 60, 10)) +
  geom_point()
grid.arrange(p1, p2, p3, ncol = 1)
```

It shows that people about 40 make the most number of contribution and highest
total contribution, but the people about 30 make the highest mean contribution. 
...
```{r echo=FALSE}
p1 <- ggplot() +
  geom_point(data = wa.amt_by_percent_white,
            aes(percent_white, n, color = 'blue')) +
  geom_point(data = wa.amt_by_percent_black,
            aes(percent_black, n, color = 'black')) +
  geom_point(data = wa.amt_by_percent_asian,
            aes(percent_asian, n, color = 'orange')) +
  geom_point(data = wa.amt_by_percent_hispanic,
            aes(percent_hispanic, n, color = 'red')) +
...
p1 <- ggplot(hillary_occupation_amt, aes(reorder(contbr_occupation, n), n)) +
  geom_bar(stat = "identity") +
  labs(title = "Hillary donator occupation") +
  coord_flip()
p2 <- ggplot(trump_occupation_amt, aes(reorder(contbr_occupation, n), n)) +
  geom_bar(stat = "identity") +
  labs(title = "Trump donator occupation") +
  coord_flip()
grid.arrange(p1, p2)
```
...
There is strong and interesting relation between
contributor occupation and candidate's backgroud.
...
The distribution of each candidate's contribution, colored by party.
It seems that Trump's have few outliers.
There are four candidates' contribution nearly accross the whole range:
Bush, Hillary, Sanders and Trump.
In other word, they have more widespread supported.
What is more, Bush's contributions are more centralized, and Sanders' are more 
polarized.
...
The distribution of each candidate's contribution, colored by gender.
...
The distribution of each candidate's contribution, colored and paired by
donator's gender.
...
The distribution of each candidate's contribution, colored by donator's gender
and faceted by median age.
...
ggplot() +
  ...
  geom_point(data = city.wa,
           aes(x = lng, y = lat, color = "grey"), alpha = 0.5) +
  scale_fill_manual(label = c('republican', 'democratic'),
                    values = c("red", "blue")) +
  scale_color_manual(label = "city", values = "grey") +
  theme_map() +
...
ggplot() +
  ...
  coord_map(projection = "mercator") +
  geom_point(data = city.wa,
           aes(x = lng, y = lat, color = "black"), alpha = 0.4) +
  scale_fill_manual(label = c('republican', 'democratic'),
                    values = c("red", "blue")) +
    scale_color_manual(label = "city", values = "black") +
  theme_map() +
  theme(legend.position = "bottom")
```

The map shows that republican receive more number of contributions by zipcode.
...
ggplot() +
  ...
  coord_map(projection = "mercator") +
  geom_point(data = city.wa,
           aes(x = lng, y = lat, color = 'black'), alpha = 0.2) +
...
The map shows that Trump receive more number of contributions by zipcode.
...
The map shows that Hillary and Trump seems matched.
